...
def nthA[T](x: Int, l: List[T]): Option[T] = (x, l) match {
  ...
  def reverseA[T](list: List[T]): List[T] = list.foldLeft(List[T]()) { (r, h) => h :: r }
  ...
  def salarySum(employees: List[Employee]): Double = employees.map(x => (x.salary)).sum
  /**
   * 5) Address list:
  ...
  def addressOf(users: List[User]): List[String] = users.map(name => name.address)
  ...
  def average(values: Iterator[Double]): Option[Double] = 
    if (values.nonEmpty)
        Option(values.foldLeft((0.0, 1)) ((acc, i) => ((acc._1 + (i - acc._1) / acc._2), acc._2 + 1))._1)
    else
        None
